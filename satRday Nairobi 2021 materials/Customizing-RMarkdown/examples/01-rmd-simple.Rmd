...
And then let's load the data. The data come from the [Amboseli Baboon Research Project](https://amboselibaboons.nd.edu/downloads/) and it has been complied by the directors of the project Jeanne Altmann and Susan C. Alberts.

```{r load-data}
amboseli_rainfall_raw <- read_excel(here::here("satRday Nairobi 2021 materials/Customizing-RMarkdown/examples/data", "amboseli_rainfall_2020.xls"), skip = 3)
```
...
One last step before plotting: let's set up a color palette inspired by the landscape of Amboseli. The color palette comes from the `ramboseli` package, which can be found [on GitHub](https://github.com/amboseli/ramboseli).
...
We're finally ready to visualize the data. Since we're interested in total yearly rainfall for the two seasons in each year, we'll summarize the data obtain these two values for each year first, and then plot the data in the summary table. We can do this all in a single pipeline.
